...
db.push(`isimler.${member.id}`, `\`${nick}\` (<@&${roller.erkek}>)`)

member.setNickname(`${kayıttabasagelecektag} ${nick}`) 
message.react(Config.onay)
member.roles.add(roller.erkek)
member.roles.add(roller.erkek)
member.roles.add(roller.erkek)
...
.setDescription(`${member} Adlı Kullanıcı <@&${roller.erkek}> Olarak Kayıt Edildi ${Config.onaye}

\`.isimler @Kagez/ID İle Kontrol Ederek Kayıt Etmeniz Önerilir \` `)
